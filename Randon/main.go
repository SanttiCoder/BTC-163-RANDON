...
const (
	numGoroutines = 8
	keyLength     = 64 // Tamanho da chave hexadecimal
)
...
func GenerateAndSendKeys(pattern string, keysChan chan<- string, stopSignal chan struct{}) {
	for {
		...
		case <-stopSignal:
			return
		default:
			randomKey := generateRandomKey(pattern)
			if !hasRepeatedCharacters(randomKey) {
				keysChan <- randomKey
			}
		}
	}
...
func SearchInKeys(wallets []string, keysChan <-chan string, stopSignal chan struct{}, startTime time.Time, keysChecked *int64, lastKeyTested *atomic.Value, mu *sync.Mutex) {
	...
	for {
		...
		case <-ticker.C:
			mu.Lock()
			printProgress(startTime, keysChecked, lastKeyTested.Load().(string))
			mu.Unlock()
		}
	}
}
